...
public class VerifyEmailException extends GlobalException {
    ...
    private VerifyEmailException(UserErrorType type) {
        super(type);
    }

    // 2) 인증 코드가 틀렸을 때
    public static VerifyEmailException invalid() {
        return new VerifyEmailException(UserErrorType.INVALID_VERIFICATION_CODE);
    }

    // 3) 인증 코드가 만료됐을 때
    public static VerifyEmailException expired() {
        return new VerifyEmailException(UserErrorType.EXPIRED_VERIFICATION_CODE);
    }
}
